...
import com.example.concertTicket_websocket.waitingqueue.infrastructure.enums.RedisKey;
...
public class RedisPubSubListener {
    ...
    private void startTokenChannelListening() {
        ...
        topic.addListener(String.class, (channel, message) -> {
                ...
                List<String> tokens = messageParser.parseMessage(message);

                RLock lock = redissonClient.getLock(RedisKey.TOKEN_PROCESSING_LOCK_KEY.getKey());
        ...
        log.info("Started listening on Redis Pub/Sub channel: {}", RedisKey.TOKEN_PUB_SUB_CHANNEL.getKey());
    };
    ...
    private void startWaitingQueueStatusChannelListening() {
        // Redis 채널을 구독하기 위한 RTopic 객체
        RTopic topic = redissonClient.getTopic(RedisKey.WAITING_QUEUE_STATUS_PUB_SUB_CHANNEL.getKey());
        ...
        topic.addListener(String.class, (channel, message) -> {
            ...
            RLock lock = redissonClient.getLock(RedisKey.WAITING_QUEUE_STATUS_LOCK_KEY.getKey());
